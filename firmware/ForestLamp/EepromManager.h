...
#define EEPROM_REMOTEXY_ADDRESS_G             (EEPROM_REMOTEXY_ADDRESS + 3)
#define EEPROM_REMOTEXY_ADDRESS_B             (EEPROM_REMOTEXY_ADDRESS + 4)
#define EEPROM_REMOTEXY_ADDRESS_X             (EEPROM_REMOTEXY_ADDRESS + 5)
#define EEPROM_REMOTEXY_ADDRESS_Y             (EEPROM_REMOTEXY_ADDRESS + 6)
  ...
  public:
      ...
      void (*readFavoritesSettings)(), void (*saveFavoritesSettings)())
    {
      ...
      *buttonEnabled = EEPROM.read(EEPROM_ESP_BUTTON_ENABLED_ADDRESS);
      LOG.printf_P(PSTR("Loaded current mode %d, switched=%d\n"), *currentMode, *onFlag);
    }
      ...
      if (*settChanged && millis() - *eepromTimeout > EEPROM_WRITE_DELAY)
      {
        ...
        *eepromTimeout = millis();
        ...
        if (EEPROM.read(EEPROM_CURRENT_MODE_ADDRESS) != *currentMode)
        {
          ...
          LOG.printf_P(PSTR("Saved current mode %d\n"), *currentMode);
        }
        saveFavoritesSettings();
        EEPROM.commit();
